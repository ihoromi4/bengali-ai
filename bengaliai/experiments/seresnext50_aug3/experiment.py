    ...
    def get_transforms(stage: str = None, mode: str = None):
        if mode == 'train':
            return albumentations.Compose([
                # transformations
                albumentations.ShiftScaleRotate(scale_limit=0.2, rotate_limit=10, border_mode=cv2.BORDER_CONSTANT, value=0, p=1.0),
                # cut and drop
                albumentations.OneOf([
                    albumentations.Cutout(num_holes=10, max_h_size=SIZE//10, max_w_size=SIZE//10, p=1.0),
                    albumentations.CoarseDropout(max_holes=10, max_height=SIZE//10, max_width=SIZE//10, p=1.0),
                ...
                # distortion
                albumentations.OneOf([
                    albumentations.OpticalDistortion(0.6, border_mode=cv2.BORDER_CONSTANT, p=1.0),
